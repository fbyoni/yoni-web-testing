(function () {
  ...
  function warnBlock(kind, url) {
    try {
      console.warn('[net-shim] blocked external ' + kind + ':', url);
    } catch (e) {}
  }

  function sanitizeExternalAnchors(root) {
    try {
      ...
      for (var i = 0; i < anchors.length; i++) {
        ...
        if (href && !sameOrigin(href)) {
          anchors[i].setAttribute('data-external-href', href);
  ...
  function guardElementSrc(el, attr) {
    try {
      ...
      Object.defineProperty(el, attr, {
        ...
        set: function (v) {
          if (!sameOrigin(v)) {
            warnBlock(el.tagName.toLowerCase() + '.' + attr, v);
            return;
          }
  ...
  if (
    ...
    navigator.serviceWorker.register = function (scriptURL, opts) {
      if (!sameOrigin(scriptURL)) {
        warnBlock('serviceWorker.register', scriptURL);
  ...
  if (typeof window.WebSocket === 'function') {
    ...
    function ShimWS(url, protocols) {
      try {
        ...
        if (wsOrigin !== loc.origin) {
          warnBlock('WebSocket', url);
          throw new Error('blocked');
        }
      } catch (e) {
        if (e && e.message === 'blocked') throw e;
      }
      return protocols !== undefined ? new OrigWS(url, protocols) : new OrigWS(url);
    }
    ShimWS.prototype = OrigWS.prototype;
    Object.keys(OrigWS).forEach(function (k) {
      try {
        ShimWS[k] = OrigWS[k];
      } catch (e) {}
    });
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      if (anchor && !sameOrigin(anchor.getAttribute('href'))) {
        warnBlock('navigation', anchor.getAttribute('href'));
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () {
      sanitizeExternalAnchors(document);
    });
  } else {
    sanitizeExternalAnchors(document);
  }
    ...
    var observer = new MutationObserver(function (mutations) {
      for (var i = 0; i < mutations.length; i++) {
        for (var j = 0; j < mutations[i].addedNodes.length; j++) {
          sanitizeExternalAnchors(mutations[i].addedNodes[j]);
        }
      }
      sanitizeExternalAnchors(document);
    });
