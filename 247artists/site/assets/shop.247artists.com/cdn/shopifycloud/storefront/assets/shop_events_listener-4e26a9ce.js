(() => {
  var AddToCartListener = class _AddToCartListener {
    ...
    static XHR_FALLBACK_STATE = { UNSENT: 0, OPENED: 1, HEADERS_RECEIVED: 2, LOADING: 3, DONE: 4 };
    ...
    static parseBlobToJson(blob, parsedPayloadHandler) {
      ...
      reader.addEventListener("loadend", () =>
        parsedPayloadHandler(JSON.parse(String.fromCharCode(...new Uint8Array(reader.result)))),
      );
  ...
  (function () {
    ...
    function productTrackProperties(variantId) {
      ...
      if (window.ShopifyAnalytics.meta.products) {
        ...
        ({ product, variant: productVariant } = findProductAndVariantInCollection(
          variantId,
          products,
        ));
    ...
    for (const entry of itemsMap.entries()) {
      ...
      if (existingItem) {
        existingItem.quantity = (
          parseInt(existingItem.quantity || "1", 10) + parseInt(item.quantity || "1", 10)
        ).toString();
      } else {
