...
export class DragZoomWrapper extends HTMLElement {
  ...
  connectedCallback() {
    this.#image &&
      ...
      isMobileBreakpoint() && (this.#initEventListeners(), this.#updateTransform()));
  }
  #initResizeListener() {
  ...
  disconnectedCallback() {
    (this.#controller.abort(), this.#resizeObserver.disconnect(), this.#cancelAnimationFrame());
  }
  #handleResize = () => {
    (!this.#initialized && isMobileBreakpoint() && this.#initEventListeners(),
      this.#initialized && this.#requestUpdateTransform());
  };
  #resizeObserver = new ResizeObserver(this.#handleResize);
  #handleTouchStart = (event) => {
    if ((preventDefault(event), event.touches.length === 2)) {
  ...
  #handleTouchMove = (event) => {
    ...
    const isZooming = event.touches.length === 2,
      isDragging = event.touches.length === 1 && this.#isDragging;
    isZooming ? this.#processZoomGesture(event) : isDragging && this.#processDragGesture(event);
  };
  #processZoomGesture(event) {
    const [point1, point2] = Array.from(event.touches).map(touchToPoint);
    if (!point1 || !point2) return;
    const currentMidpoint = getMidpoint(point1, point2),
      currentDistance = getDistance(point1, point2),
      oldScale = this.#scale,
      newScale = (currentDistance / this.#initialDistance) * this.#startScale;
    this.#scale = clamp(newScale, MIN_ZOOM, MAX_ZOOM);
    const containerCenter = { x: this.clientWidth / 2, y: this.clientHeight / 2 },
      distanceFromCenter = {
        x: currentMidpoint.x - containerCenter.x,
        y: currentMidpoint.y - containerCenter.y,
      },
      scaleDelta = this.#scale / oldScale - 1;
  ...
  #handleTouchEnd = (event) => {
    event.touches.length === 0 && ((this.#isDragging = !1), this.#requestUpdateTransform());
  };
  #getMinZoom = () => {
  ...
  #getImageDimensions = () => {
    ...
    return (
      naturalAspect > containerAspect
        ? ((imageWidth = containerRect.width), (imageHeight = imageWidth / naturalAspect))
        : ((imageHeight = containerRect.height), (imageWidth = imageHeight * naturalAspect)),
      imageWidth === 0 || imageHeight === 0 ? null : { width: imageWidth, height: imageHeight }
    );
  };
  #constrainTranslation() {
    ...
    const { width: imageWidth, height: imageHeight } = imageDimensions,
      ...
      excessWidth = Math.max(0, scaledWidth - containerWidth),
      excessHeight = Math.max(0, scaledHeight - containerHeight),
      maxTranslateX = excessWidth / (2 * this.#scale),
      maxTranslateY = excessHeight / (2 * this.#scale);
  ...
  #requestUpdateTransform = () => {
    this.#animationFrame || (this.#animationFrame = requestAnimationFrame(this.#updateTransform));
  };
  ...
  destroy() {
    (this.#controller.abort(), this.#cancelAnimationFrame());
  }
}
customElements.get("drag-zoom-wrapper") ||
...
//# sourceMappingURL=/cdn/shop/t/3/assets/drag-zoom-wrapper.js.map?v=139025577638372858751754421400
