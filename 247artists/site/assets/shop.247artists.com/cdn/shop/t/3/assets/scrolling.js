...
export class Scroller {
  ...
  async to(input, options) {
    ...
    this.element[`scroll${this.#edge}`] !== value
      ? this.#scroll({ ...options, method: "scrollTo", value })
      : this.#isScrolling && this.#handleScrollEnd(!1);
  }
  by(value, options) {
  ...
  #scroll(options) {
    ...
    (this.#reset(), (this.#ignore = instant), (this.#userEvent = !1));
    const currentPosition = this.element[`scroll${this.#edge}`],
      targetPosition = method === "scrollBy" ? currentPosition + value : value;
    if (Math.abs(targetPosition - currentPosition) < 1) return Promise.resolve();
    (instant || this.#setup(),
      this.element[method]({
  ...
  #setup() {
    this.#promise || (this.#promise = new Promise((resolve) => (this.#resolve = resolve)));
  }
  #reset = () => {
  ...
  #handleScroll = () => {
    if ((this.#isScrolling || (this.#onScrollInit?.(), (this.#isScrolling = !0)), this.#ignore)) {
      (this.#reset(), this.#handleScrollEnd(!1));
      return;
    }
    const userEvent = this.#userEvent;
    (this.#setup(), userEvent && this.#throttledCallback(), this.#handleScrollEnd(userEvent));
  };
  #handleScrollEnd = debounce((userEvent) => {
    (this.#resolve?.(),
      userEvent && this.#endCallback(),
      this.#reset(),
      this.#isScrolling && (this.#onScrollEnd?.(), (this.#isScrolling = !1)));
  }, SCROLL_END_TIMEOUT);
  set snap(value) {
    ((this.#ignore = !0),
...
function calculatePaddingStart(element, axis) {
  const computedStyle = getComputedStyle(element),
    value = axis === "x" ? computedStyle.paddingInlineStart : computedStyle.paddingBlockStart;
...
export function scrollIntoView(
  ...
  if (!ancestor) return element.scrollIntoView({ behavior, block, inline });
...
class ScrollHint extends HTMLElement {
  connectedCallback() {
    (this.addEventListener("scroll", this.#update), this.#resizeObserver.observe(this));
  }
  disconnectedCallback() {
    (this.removeEventListener("scroll", this.#update), this.#resizeObserver.disconnect());
  }
  #update = () => {
    const { scrollTop, scrollHeight, clientHeight, scrollLeft, scrollWidth, clientWidth } = this,
      scrollDirection = scrollWidth > clientWidth ? "horizontal" : "vertical",
      scrollPercentage =
...
customElements.get("scroll-hint") || customElements.define("scroll-hint", ScrollHint);
//# sourceMappingURL=/cdn/shop/t/3/assets/scrolling.js.map?v=10957690925586738441754421400
