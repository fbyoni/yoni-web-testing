...
class CartDiscount extends Component {
  ...
  applyDiscount = async (event) => {
    ...
    try {
      ...
      const config = fetchConfig("json", {
        ...
        data = await (
          await fetch(Theme.routes.cart_update_url, { ...config, signal: abortController.signal })
        ).json();
      if (
        data.discount_codes.find(
          (discount) => discount.code === discountCodeValue && discount.applicable === !1,
        )
      ) {
      ...
      const newHtml = data.sections[this.dataset.sectionId],
        section = new DOMParser()
          ...
          .getElementById(`shopify-section-${this.dataset.sectionId}`),
        discountCodes = section?.querySelectorAll(".cart-discount__pill") || [];
      if (section) {
        ...
        if (
          ...
          data.discount_codes.find(
            (discount) => discount.code === discountCodeValue && discount.applicable === !0,
          )
        ) {
  ...
  #handleDiscountError(type) {
    const { cartDiscountError, cartDiscountErrorDiscountCode, cartDiscountErrorShipping } =
        this.refs,
      target = type === "discount_code" ? cartDiscountErrorDiscountCode : cartDiscountErrorShipping;
    (cartDiscountError.classList.remove("hidden"), target.classList.remove("hidden"));
  }
  #existingDiscounts() {
    const discountCodes = [],
...
customElements.get("cart-discount-component") ||
  customElements.define("cart-discount-component", CartDiscount);
//# sourceMappingURL=/cdn/shop/t/3/assets/cart-discount.js.map?v=107077853990124422021754421401
