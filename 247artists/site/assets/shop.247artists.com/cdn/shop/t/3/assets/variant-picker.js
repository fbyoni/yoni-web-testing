...
export default class VariantPicker extends Component {
  ...
  #abortController;
  connectedCallback() {
    (super.connectedCallback(), this.addEventListener("change", this.variantChanged.bind(this)));
  }
  ...
  updateSelectedOption(target) {
    ...
    if (
      (target instanceof HTMLInputElement && (target.checked = !0),
  ...
  buildRequestUrl(selectedOption, source = null, sourceSelectedOptionsValues = []) {
    ...
    const params = [];
    return (
      this.selectedOptionsValues.length && !source
        ? params.push(`option_values=${this.selectedOptionsValues.join(",")}`)
        : source === "product-card" &&
          (this.selectedOptionsValues.length
            ? params.push(`option_values=${sourceSelectedOptionsValues.join(",")}`)
            : params.push(`option_values=${selectedOption.dataset.optionValueId}`)),
      this.closest("quick-add-component") || this.closest("swatches-variant-picker-component")
        ? (productUrl?.includes("?") && (productUrl = productUrl.split("?")[0]),
          `${productUrl}?section_id=section-rendering-product-card&${params.join("&")}`)
        : `${productUrl}?${params.join("&")}`
    );
  }
  fetchUpdatedSection(requestUrl, shouldMorphMain = !1) {
    (this.#abortController?.abort(),
      ...
      fetch(requestUrl, { signal: this.#abortController.signal })
        ...
        .then((responseText) => {
          ...
          if (textContent)
            if (shouldMorphMain) this.updateMain(html);
            else {
...
//# sourceMappingURL=/cdn/shop/t/3/assets/variant-picker.js.map?v=113536721324242252251754935551
