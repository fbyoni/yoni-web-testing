...
const MORPH_OPTIONS = {
  ...
  reject(oldNode, newNode) {
    return !!(
      (newNode.nodeType === Node.TEXT_NODE && newNode.nodeValue?.trim() === "") ||
  ...
  onBeforeUpdate(oldNode, newNode) {
    if (oldNode instanceof Element && newNode instanceof Element) {
      ...
      for (const attribute of attributes) {
        ...
        oldValue && oldValue !== newValue && newNode.setAttribute(attribute, oldValue);
      }
  ...
  onAfterUpdate(node) {
    node instanceof Component && queueMicrotask(() => node.updatedCallback());
  },
};
export function morph(oldTree, newTree, options = MORPH_OPTIONS) {
  if (!oldTree || !newTree) throw new Error("Both oldTree and newTree must be provided");
  ...
  if (options.childrenOnly) return (updateChildren(newTree, oldTree, options), oldTree);
...
function updateNode(newNode, oldNode, options) {
  if (
    ...
    ((newNode instanceof HTMLDetailsElement && oldNode instanceof HTMLDetailsElement) ||
      ...
      (newNode.hasAttribute("declarative-open") || (newNode.open = oldNode.open)),
  ...
  (newNode instanceof Element && oldNode instanceof Element
    ...
    newNode instanceof HTMLInputElement && oldNode instanceof HTMLInputElement
      ...
      : newNode instanceof HTMLOptionElement && oldNode instanceof HTMLOptionElement
...
function getNodeKey(node, options) {
  return options?.getNodeKey?.(node) ?? (node instanceof Element ? node.id : void 0);
}
function updateAttribute(newNode, oldNode, name) {
  newNode[name] !== oldNode[name] &&
    ((oldNode[name] = newNode[name]),
    newNode[name] != null ? oldNode.setAttribute(name, "") : oldNode.removeAttribute(name));
}
...
function updateInput(newNode, oldNode) {
  ...
  (updateAttribute(newNode, oldNode, "checked"),
    ...
    oldNode.type !== "file" &&
      ...
      newValue === "null" && ((oldNode.value = ""), oldNode.removeAttribute("value")),
...
function same(a, b, options) {
  ...
  if (a.nodeType === Node.ELEMENT_NODE) {
    if (a instanceof Element && b instanceof Element && a.tagName !== b.tagName) return !1;
...
//# sourceMappingURL=/cdn/shop/t/3/assets/morph.js.map?v=130689995393408371391754421400
