...
export const viewTransition = { current: void 0 };
const viewTransitionTypes = {
  "product-grid": async () => {
    const grid = document.querySelector(".product-grid"),
      productCards = [...document.querySelectorAll(".product-grid .product-grid__item")];
      ...
      if (types) {
        for (const type of types)
          if (viewTransitionTypes[type]) {
            ...
            cleanupFunction && cleanupFunctions.push(cleanupFunction);
          }
      }
      const transition = document.startViewTransition(callback);
      (viewTransition.current || (viewTransition.current = transition.finished),
        types && types.forEach((type) => transition.types.add(type)),
        transition.finished.then(() => {
          ((viewTransition.current = void 0),
...
export function fetchConfig(type = "json", config = {}) {
  ...
  return (
    type === "javascript" &&
      ((headers["X-Requested-With"] = "XMLHttpRequest"), delete headers["Content-Type"]),
...
export function debounce(fn, wait) {
  ...
  function debounced(...args) {
    (clearTimeout(timeout), (timeout = setTimeout(() => fn.apply(this, args), wait)));
  }
  return (
    (debounced.cancel = () => {
...
export function throttle(fn, delay) {
  ...
  function throttled(...args) {
    ...
    now - lastCall >= delay && ((lastCall = now), fn.apply(this, args));
  }
  return (
    (throttled.cancel = () => {
...
export function formatMoney(value) {
  ...
  return valueWithNoSpaces.indexOf(",") === -1
    ? valueWithNoSpaces
    : valueWithNoSpaces.indexOf(",") < valueWithNoSpaces.indexOf(".")
      ? valueWithNoSpaces.replace(",", "")
      : valueWithNoSpaces.indexOf(".") < valueWithNoSpaces.indexOf(",")
        ? valueWithNoSpaces.replace(".", "").replace(",", ".")
        : valueWithNoSpaces.indexOf(",") !== -1
          ? valueWithNoSpaces.replace(",", ".")
          : valueWithNoSpaces;
}
...
export function onAnimationEnd(elements, callback, options = { subtree: !0 }) {
  const animationPromises = (
    ...
    (acc, animation) => (
      animation.timeline instanceof DocumentTimeline && acc.push(animation.finished),
      acc
    ),
    [],
  );
...
function getCardsToAnimate(grid, cards) {
  ...
  const gridRect = grid.getBoundingClientRect(),
    ...
    visibleHeight = Math.round(visibleArea.bottom - visibleArea.top);
  ...
  const cardGap =
    ...
    gridGap = parseInt(gridStyle.getPropertyValue("--product-grid-gap")) || 12,
    detailsSize = ((parseInt(gridStyle.fontSize) || 16) + 2) * 2,
    ...
    rowsInGrid = Math.ceil((visibleHeight - gridGap) / (cardHeight + gridGap));
...
customElements.get("text-component") || customElements.define("text-component", TextComponent);
export function resetShimmer(container = document.body) {
  container.querySelectorAll("[shimmer]").forEach((item) => item.removeAttribute("shimmer"));
}
export function changeMetaThemeColor(colorSourceElement) {
  const metaThemeColor = document.head.querySelector('meta[name="theme-color"]'),
    containerStyle = window.getComputedStyle(colorSourceElement);
  metaThemeColor && metaThemeColor.setAttribute("content", containerStyle.backgroundColor);
}
class Scheduler {
  #queue = new Set();
  #scheduled = !1;
  ...
  flush = () => {
    ...
    (this.#queue.clear(), (this.#scheduled = !1));
  };
}
export const scheduler = new Scheduler();
Theme.utilities = { ...Theme.utilities, scheduler };
//# sourceMappingURL=/cdn/shop/t/3/assets/utilities.js.map?v=26927751642168286221754421400
