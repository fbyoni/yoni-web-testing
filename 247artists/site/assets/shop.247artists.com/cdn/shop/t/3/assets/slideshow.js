...
export class Slideshow extends Component {
    ...
    name === "initial-slide" &&
      ...
      queueMicrotask(() => {
        ...
        const index = parseInt(newValue, 10) || 0,
          slide_id = this.refs.slides[index]?.getAttribute("slide-id");
        slide_id && this.select({ id: slide_id }, void 0, { animate: !1 });
      });
  }
  ...
  async connectedCallback() {
    if (
      ...
      viewTransition.current && (await viewTransition.current, !this.isConnected))
    )
      return;
    ...
    ((this.#scroll = new Scroller(scroller, {
      ...
      scheduler.schedule(() => {
        ...
        (this.initialSlideIndex !== 0 && initialSlideId
          ? (this.select({ id: initialSlideId }, void 0, { animate: !1 }),
            (visibleSlidesAmount = 1))
          : ((visibleSlidesAmount = this.#updateVisibleSlides()),
            visibleSlidesAmount === 0 &&
              (this.select(0, void 0, { animate: !1 }), (visibleSlidesAmount = 1))),
          (this.#resizeObserver = new ResizeObserver(async () => {
            (viewTransition.current && (await viewTransition.current),
              visibleSlidesAmount > 1 && this.#updateVisibleSlides(),
              this.hasAttribute("auto-hide-controls") && this.#updateControlsVisibility());
          })),
  ...
  async select(input, event, options = {}) {
    ...
    for (const slide2 of this.refs.slides)
      slide2.hasAttribute("reveal") &&
        (slide2.removeAttribute("reveal"), slide2.setAttribute("aria-hidden", "true"));
    let requestedIndex = (() => {
      if (typeof input == "number") return input;
      if (typeof input == "string") return parseInt(input, 10);
      if ("id" in input) {
        const requestedSlide = this.refs.slides.find(
          (slide2) => slide2.getAttribute("slide-id") == input.id,
        );
        return !requestedSlide || !this.slides
          ? void 0
          : (requestedSlide.hasAttribute("hidden") &&
              (requestedSlide.setAttribute("reveal", ""),
              requestedSlide.setAttribute("aria-hidden", "false")),
            this.slides.indexOf(requestedSlide));
      }
    })();
    const { current } = this;
    if (requestedIndex === void 0 || isNaN(requestedIndex) || requestedIndex === current) return;
    const { slides } = this;
    if (!slides?.length) return;
    (this.infinite || (requestedIndex = clamp(requestedIndex, 0, slides.length - 1)),
      event?.preventDefault());
    const { animate = !0 } = options,
      lastIndex = slides.length - 1;
    let index = requestedIndex;
    requestedIndex < 0 ? (index = lastIndex) : requestedIndex > lastIndex && (index = 0);
    const isAdjacentSlide =
        Math.abs(index - current) <= 1 && requestedIndex >= 0 && requestedIndex <= lastIndex,
      { visibleSlides } = this,
      instant = prefersReducedMotion() || !animate;
    if (!instant && !isAdjacentSlide && visibleSlides.length === 1) {
      ((this.#disabled = !0), await this.#scroll.finished);
      ...
      (targetSlide.before(placeholder),
        ...
        queueMicrotask(async () => {
          (await this.#scroll.finished,
            ...
            this.#scroll.to(targetSlide, { instant: !0 }));
        }));
    }
  ...
  next(event, options) {
    (event?.preventDefault(), this.select(this.nextIndex, event, options));
  }
  previous(event, options) {
    (event?.preventDefault(), this.select(this.previousIndex, event, options));
  }
  ...
  set current(value) {
    ...
    ((this.#current = value), current && (current.textContent = `${value + 1}`));
    for (const controls of [thumbnails, dots])
      controls?.forEach((el, i) => el.setAttribute("aria-selected", `${i === value}`));
    (previous && (previous.disabled = !this.infinite && value === 0),
      next && (next.disabled = !!(!this.infinite && slides && this.nextIndex >= slides.length)));
  }
  get infinite() {
  ...
  get previousIndex() {
    const { current, visibleSlides } = this,
      modifier = visibleSlides.length > 1 ? visibleSlides.length : 1;
  ...
  get atStart() {
    ...
    return slides?.length ? current === 0 : !1;
  }
  get atEnd() {
    const { current, slides } = this;
    return slides?.length ? current === slides.length - 1 : !1;
  }
  set disabled(value) {
  ...
  #onTransitionEnd = () => {
    (this.#updateVisibleSlides(), this.removeAttribute("transitioning"));
  };
  #sync = () => {
    const { slides } = this;
    ...
    const { axis } = this.#scroll,
      { scroller } = this.refs,
      centers = visibleSlides.map((slide) => center(slide, axis)),
      referencePoint =
        visibleSlides.length > 1 ? scroller.getBoundingClientRect()[axis] : center(scroller, axis),
      closestCenter = closest(centers, referencePoint),
      closestVisibleSlide = visibleSlides[centers.indexOf(closestCenter)];
  ...
  #handleMouseDown = (event) => {
    ...
    const onPointerMove = (event2) => {
        const current = event2[axis],
          initialDelta = startPosition - current;
        if (!initialDelta) return;
        if (!moved) {
          ((moved = !0),
            this.setPointerCapture(event2.pointerId),
            document.addEventListener("click", preventDefault, { once: !0, signal }));
          const movingRight = initialDelta < 0,
            movingLeft = initialDelta > 0,
            closestSlideshow = this.parentElement?.closest("slideshow-component"),
            isNested = closestSlideshow instanceof Slideshow && closestSlideshow !== this,
            cannotMoveInDirection = (movingRight && this.atStart) || (movingLeft && this.atEnd);
        ...
        ((velocity = Math.round((delta / timeDelta) * 1e3)),
          (previous = current),
          (distanceTravelled += Math.abs(delta)),
          this.#scroll.by(delta, { instant: !0 }));
      },
      onPointerUp = async (event2) => {
        controller.abort();
        const { current, slides: slides2 } = this,
          { scroller } = this.refs;
        if (((this.#dragging = !1), !slides2?.length || !scroller)) return;
        const direction = Math.sign(velocity),
          next = this.#sync(),
          modifier =
            current !== next || Math.abs(velocity) < 10 || distanceTravelled < 10 ? 0 : direction,
          newIndex = clamp(next + modifier, 0, slides2.length - 1),
          newSlide = slides2[newIndex],
        ...
        (this.#scroll.to(newSlide),
          ...
          await this.#scroll.finished,
          !this.#dragging && ((this.#scroll.snap = !0), this.resume()));
      };
    ((this.#scroll.snap = !1),
  ...
  #updateControlsVisibility() {
    ...
    slideshowControls instanceof HTMLElement &&
...
//# sourceMappingURL=/cdn/shop/t/3/assets/slideshow.js.map?v=63092151078244317761754421400
