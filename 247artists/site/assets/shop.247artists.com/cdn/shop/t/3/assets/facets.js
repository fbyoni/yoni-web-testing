...
customElements.get("facets-form-component") ||
...
class FacetInputsComponent extends Component {
  ...
  updateFilters() {
    ...
    facetsForm instanceof FacetsFormComponent &&
      (facetsForm.updateFilters(), this.#updateSelectedFacetSummary());
  }
  handleKeyDown(event) {
  ...
  #updateSelectedFacetSummary() {
    ...
    statusComponent instanceof FacetStatusComponent &&
      statusComponent.updateListSummary(checkedInputElements);
  }
}
customElements.get("facet-inputs-component") ||
...
class PriceFacetComponent extends Component {
  ...
  updatePriceFilterAndResults() {
    ...
    const facetsForm = this.closest("facets-form-component");
    facetsForm instanceof FacetsFormComponent &&
      (facetsForm.updateFilters(), this.#setMinAndMaxValues(), this.#updateSummary());
  }
...
customElements.get("price-facet-component") ||
...
class FacetClearComponent extends Component {
  ...
  clearFilter(event) {
    ...
    event.target
      .closest("facet-inputs-component, price-facet-component")
      ?.querySelectorAll('[type="checkbox"]:checked, input')
      .forEach((input) => {
        input instanceof HTMLInputElement && ((input.checked = !1), (input.value = ""));
      });
    const statusComponent = event.target
      .closest("details")
      ?.querySelector("facet-status-component");
  ...
  #handleKeyUp = (event) => {
    event.metaKey || (event.key === "Enter" && this.clearFilter(event));
  };
  #handleFilterUpdate = (event) => {
    const { clearButton } = this.refs;
    clearButton instanceof Element &&
      clearButton.classList.toggle("facets__clear--active", event.shouldShowClearAll());
  };
}
customElements.get("facet-clear-component") ||
...
customElements.get("facet-remove-component") ||
...
class SortingFilterComponent extends Component {
    ...
    switch (event.key) {
      ...
      case " ":
        if (event.target instanceof Element) {
          ...
          targetOption && (event.preventDefault(), this.#selectOption(targetOption));
        }
        break;
      case "Escape":
        (event.preventDefault(), this.#closeDropdown());
        break;
    }
  };
  handleToggle = () => {
    ...
    if (
      ...
      isOpen && listbox instanceof Element)
    ) {
      const selectedOption = listbox.querySelector('[aria-selected="true"]');
      selectedOption instanceof HTMLElement && selectedOption.focus();
    }
  };
  #moveFocus(options, newIndex) {
    options.forEach((option) => {
      option instanceof HTMLElement && (option.tabIndex = -1);
    });
    const targetOption = options[newIndex];
    targetOption instanceof HTMLElement && ((targetOption.tabIndex = 0), targetOption.focus());
  }
  #selectOption(option) {
  ...
  #closeDropdown() {
    ...
    if (details instanceof HTMLDetailsElement) {
      const options = this.querySelectorAll('[role="option"]'),
        selectedOption = this.querySelector('[aria-selected="true"]');
      (options.forEach((opt) => {
        opt instanceof HTMLElement && (opt.tabIndex = -1);
      }),
        selectedOption instanceof HTMLElement && (selectedOption.tabIndex = 0),
        (details.open = !1),
  ...
  updateFilterAndSorting(event) {
    ...
    const isMobile = window.innerWidth < 750,
      shouldDisable = this.dataset.shouldUseSelectOnMobile === "true";
    if (shouldDisable)
      if (isMobile)
        this.querySelectorAll('input[name="sort_by"]').forEach((input) => {
          input instanceof HTMLInputElement && (input.disabled = !0);
        });
      else {
    ...
    if ((facetsForm.updateFilters(), this.updateFacetStatus(event), shouldDisable))
      if (isMobile)
        this.querySelectorAll('input[name="sort_by"]').forEach((input) => {
          input instanceof HTMLInputElement && (input.disabled = !1);
        });
      else {
        const selectElement = this.querySelector('select[name="sort_by"]');
        if (!(selectElement instanceof HTMLSelectElement)) return;
...
class FacetStatusComponent extends Component {
  ...
  #updateSwatchSummary(checkedInputElements, checkedInputElementsCount) {
    ...
    if (
      (facetStatus.classList.remove("bubble", "facets__bubble"), checkedInputElementsCount === 0)
    ) {
    ...
    facetStatus.innerHTML = Array.from(checkedInputElements)
      .map(
        (inputElement) => inputElement.parentElement?.querySelector("span.swatch")?.outerHTML ?? "",
      )
      .join("");
  }
  ...
  updatePriceSummary(minInput, maxInput) {
    const minInputValue = minInput.value,
      maxInputValue = maxInput.value,
      { facetStatus } = this.refs;
    if (!minInputValue && !maxInputValue) {
      facetStatus.innerHTML = "";
      return;
    }
    const minInputNum = this.#parseCents(minInputValue, "0"),
      maxInputNum = this.#parseCents(maxInputValue, facetStatus.dataset.rangeMax);
  ...
  #parseCents(value, fallback = "0") {
    const parts = value ? value.trim().split(/[^0-9]/) : (parseInt(fallback, 10) / 100).toString(),
      [wholeStr, fractionStr, ...rest] = parts;
  ...
  #formatMoney(moneyValue) {
    ...
    const template = this.refs.moneyFormat.content.textContent || "{{amount}}",
      currency = this.refs.facetStatus.dataset.currency || "";
    return template.replace(/{{\s*(\w+)\s*}}/g, (_, placeholder) => {
      ...
      let thousandsSeparator = ",",
        decimalSeparator = ".",
        precision = CURRENCY_DECIMALS[currency.toUpperCase()] ?? DEFAULT_CURRENCY_DECIMALS;
      return (
        placeholder === "amount" ||
              ...
              : placeholder === "amount_no_decimals_with_comma_separator"
                ...
                : placeholder === "amount_no_decimals_with_space_separator"
                  ...
                  : placeholder === "amount_with_space_separator"
                    ? ((thousandsSeparator = " "), (decimalSeparator = ","))
                    : placeholder === "amount_with_period_and_space_separator"
                      ? ((thousandsSeparator = " "), (decimalSeparator = "."))
                      : placeholder === "amount_with_apostrophe_separator" &&
                        ((thousandsSeparator = "'"), (decimalSeparator = "."))),
        this.#formatCents(moneyValue, thousandsSeparator, decimalSeparator, precision)
      );
    });
  }
...
customElements.get("facet-status-component") ||
...
//# sourceMappingURL=/cdn/shop/t/3/assets/facets.js.map?v=16510020039899366041754421400
