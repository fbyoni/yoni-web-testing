...
class DeferredMedia extends Component {
  ...
  showDeferredMedia = () => {
    (this.loadContent(!0), (this.isPlaying = !0), this.updatePlayPauseHint(this.isPlaying));
  };
  loadContent(focus = !0) {
    if (this.getAttribute("data-media-loaded")) return;
    this.dispatchEvent(new MediaStartedPlayingEvent(this));
    const content = this.querySelector("template")?.content.firstElementChild?.cloneNode(!0);
    content &&
      (this.setAttribute("data-media-loaded", "true"),
      this.appendChild(content),
      focus && content instanceof HTMLElement && content.focus(),
      this.refs.deferredMediaPlayButton?.classList.add("deferred-media__playing"),
      content instanceof HTMLVideoElement && content.getAttribute("autoplay") && content.play());
  }
  toggleMedia() {
  ...
  pauseMedia() {
    ...
    (iframe
      ...
      (this.isPlaying = !1),
      this.getAttribute("data-media-loaded") && this.updatePlayPauseHint(this.isPlaying));
  }
}
...
class ProductModel extends DeferredMedia {
  ...
  pauseMedia() {
    (super.pauseMedia(), this.modelViewerUI?.pause());
  }
  playMedia() {
    (super.playMedia(), this.modelViewerUI?.play());
  }
  async setupModelViewerUI(errors) {
    if (
      errors ||
      (Shopify.ModelViewerUI || (await this.#waitForModelViewerUI()), !Shopify.ModelViewerUI)
    )
      return;
    const element = this.querySelector("model-viewer");
    if (!element) return;
    const signal = this.#abortController.signal;
    if (((this.modelViewerUI = new Shopify.ModelViewerUI(element)), !this.modelViewerUI)) return;
    this.playMedia();
    let pointerStartX = 0,
      pointerStartY = 0;
    (element.addEventListener(
      ...
      element.addEventListener(
        ...
        (event) => {
          ...
          Math.sqrt(distanceX * distanceX + distanceY * distanceY) < 10 && this.pauseMedia();
        },
        { signal },
      ));
  }
...
//# sourceMappingURL=/cdn/shop/t/3/assets/media.js.map?v=37825095869491791421754421400
