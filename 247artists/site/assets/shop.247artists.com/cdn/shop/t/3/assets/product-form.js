...
export const ADD_TO_CART_TEXT_ANIMATION_DURATION = 2e3;
export class AddToCartComponent extends Component {
  requiredRefs = ["addToCartButton"];
  #animationTimeout;
  #cleanupTimeout;
  connectedCallback() {
    (super.connectedCallback(), this.addEventListener("pointerenter", this.#preloadImage));
  }
  ...
  handleClick(event) {
    (this.animateAddToCart(), event.target.closest(".quick-add-modal") || this.#animateFlyToCart());
  }
  #preloadImage = () => {
  ...
  #animateFlyToCart() {
    const { addToCartButton } = this.refs,
      cartIcon = document.querySelector(".header-actions__cart-icon"),
      image = this.dataset.productVariantMedia;
  ...
  animateAddToCart() {
    ...
    (this.#animationTimeout && clearTimeout(this.#animationTimeout),
      this.#cleanupTimeout && clearTimeout(this.#cleanupTimeout),
      addToCartButton.classList.contains("atc-added") || addToCartButton.classList.add("atc-added"),
...
customElements.get("add-to-cart-component") ||
...
class ProductFormComponent extends Component {
  ...
  handleSubmit(event) {
    ...
    if (
      ...
      this.refs.addToCartButtonContainer?.refs.addToCartButton?.getAttribute("disabled") === "true")
    )
      return;
        ...
        if (response.status) {
          ...
          (this.#setLiveRegionText(response.message),
            (this.#timeout = setTimeout(() => {
              addToCartTextError &&
                (addToCartTextError.classList.add("hidden"), this.#clearLiveRegionText());
            }, 1e4)),
            this.dispatchEvent(
          ...
          if (this.refs.addToCartButtonContainer?.refs.addToCartButton) {
            const addedText =
              this.refs.addToCartButtonContainer.refs.addToCartButton
                .querySelector(".add-to-cart-text--added")
                ?.textContent?.trim() || Theme.translations.added;
  ...
  #onVariantUpdate = (event) => {
    ...
    const { variantId, addToCartButtonContainer } = this.refs,
      ...
      newAddToCartButton = event.detail.data.html.querySelector('[ref="addToCartButton"]');
    if (
      currentAddToCartButton &&
      (event.detail.resource == null || event.detail.resource.available == !1
      ...
      newAddToCartButton && morph(currentAddToCartButton, newAddToCartButton),
...
customElements.get("product-form-component") ||
...
class FlyToCart extends HTMLElement {
  ...
  #animate() {
    const rect = this.getBoundingClientRect(),
      ...
      destinationRect = this.destination.getBoundingClientRect(),
      ...
      controlPoint1 = { x: startPoint.x, y: startPoint.y - 200 },
      controlPoint2 = { x: endPoint.x - 300, y: endPoint.y - 100 };
    let startTime = null;
    const duration = 600;
    ...
    const animate = (currentTime) => {
      ...
      const elapsed = currentTime - startTime,
        ...
        position = bezierPoint(progress, startPoint, controlPoint1, controlPoint2, endPoint);
      ...
      (this.style.setProperty("--scale", `${scale}`),
        progress < 1
          ...
          : ((this.style.opacity = "0"), onAnimationEnd(this, () => this.remove())));
    };
...
customElements.get("fly-to-cart") || customElements.define("fly-to-cart", FlyToCart);
//# sourceMappingURL=/cdn/shop/t/3/assets/product-form.js.map?v=141380535127643269961754421400
