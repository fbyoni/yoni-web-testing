...
import { ThemeEvents, SlideshowSelectEvent } from "@theme/events";
...
export class ProductCard extends Component {
  ...
  getSelectedVariantId() {
    return (
      this.querySelector('input[type="radio"]:checked[data-variant-id]')?.dataset.variantId || null
    );
  }
  getProductCardLink() {
  ...
  connectedCallback() {
    if ((super.connectedCallback(), !(this.refs.productCardLink instanceof HTMLAnchorElement)))
    ...
    (this.#handleQuickAdd(),
      ...
      setTimeout(() => {
        this.refs.slideshow?.isNested && this.#preloadNextPreviewImage();
      }));
  }
  disconnectedCallback() {
    (super.disconnectedCallback(), this.removeEventListener("click", this.navigateToProduct));
  }
  #preloadNextPreviewImage() {
    this.refs.slideshow?.slides?.[this.refs.slideshow?.current]?.nextElementSibling
  ...
  #updateOverflowList() {
    const overflowList = this.querySelector("swatches-variant-picker-component overflow-list"),
      isActiveOverflowList = !!overflowList?.querySelector('[slot="overflow"]');
    !overflowList ||
      !isActiveOverflowList ||
      requestAnimationFrame(() => {
        overflowList.dispatchEvent(new CustomEvent("reflow", { bubbles: !0, detail: {} }));
      });
  }
  updatePrice(event) {
    const priceContainer = this.querySelectorAll("product-price [ref='priceContainer']")[1],
      newPriceElement = event.detail.data.html.querySelector(
        "product-price [ref='priceContainer']",
      );
    newPriceElement && priceContainer && morph(priceContainer, newPriceElement);
  }
  #updateProductUrl(event) {
    ...
    if (
      ...
      ((productCardLink.href = productUrl),
        cardGalleryLink instanceof HTMLAnchorElement && (cardGalleryLink.href = productUrl),
        productTitleLink instanceof HTMLAnchorElement && (productTitleLink.href = productUrl));
    }
  }
  #isUnavailableVariantSelected(event) {
    const allVariants = event.detail.data.html.querySelectorAll("input:checked");
  ...
  #toggleAddToCartButton(enable) {
    ...
    addToCartButton instanceof HTMLButtonElement && (addToCartButton.disabled = !enable);
  }
  #updateVariantImages() {
    ...
    if (slideshow && selectedImageId) {
      ...
      for (const slide of slides)
        slide.getAttribute("variant-image") != null &&
          (slide.hidden = slide.getAttribute("slide-id") !== selectedImageId);
      slideshow.select({ id: selectedImageId }, void 0, { animate: !1 });
    }
  }
  get allVariants() {
  ...
  #handleSlideshowSelect = (event) => {
    event.detail.userInitiated && (this.#previousSlideIndex = event.detail.index);
  };
  previewVariant(id) {
    const { slideshow } = this.refs;
    slideshow && (this.resetVariant.cancel(), slideshow.select({ id }, void 0, { animate: !1 }));
  }
  previewImage(event) {
    ...
    slideshow &&
      ...
      this.#previousSlideIndex != null && this.#previousSlideIndex > 0
        ? slideshow.select(this.#previousSlideIndex, void 0, { animate: !1 })
        : (slideshow.next(void 0, { animate: !1 }),
          setTimeout(() => this.#preloadNextPreviewImage())));
  }
  resetImage(event) {
    if (event.pointerType !== "mouse") return;
    const { slideshow } = this.refs;
    if (this.variantPicker) this.#resetVariant();
    else {
      if (!slideshow) return;
      slideshow.previous(void 0, { animate: !1 });
    }
  }
  #resetVariant = () => {
    const { slideshow } = this.refs;
    ...
    if (this.variantPicker?.selectedOption) {
      ...
      if (id) {
        slideshow.select({ id }, void 0, { animate: !1 });
        return;
      }
    }
    const initialSlide = slideshow.initialSlide,
      slideId = initialSlide?.getAttribute("slide-id");
    if (initialSlide && slideshow.slides?.includes(initialSlide) && slideId) {
      slideshow.select({ id: slideId }, void 0, { animate: !1 });
      return;
    }
    slideshow.previous(void 0, { animate: !1 });
  };
  navigateToProduct = (event) => {
    ...
    const linkURL = new URL(link.href),
      productCardAnchor = link.getAttribute("id");
    if (!productCardAnchor) return;
    const url = new URL(window.location.href),
      parent = this.closest("li");
    ((url.hash = productCardAnchor),
      parent && parent.dataset.page && url.searchParams.set("page", parent.dataset.page),
...
class SwatchesVariantPickerComponent extends VariantPicker {
  ...
  variantChanged(event) {
    ...
    const isSwatchInput =
      ...
      firstAvailableVariantId = clickedSwatch.dataset.firstAvailableOrFirstVariantId;
    if (isSwatchInput && availableCount > 0 && firstAvailableVariantId) {
      ...
      ((this.pendingVariantId = firstAvailableVariantId), this.fetchUpdatedSection(requestUrl));
      return;
    }
  ...
  showAllSwatches(event) {
    ...
    overflowList instanceof OverflowList && overflowList.showAll();
  }
}
customElements.get("swatches-variant-picker-component") ||
  customElements.define("swatches-variant-picker-component", SwatchesVariantPickerComponent);
//# sourceMappingURL=/cdn/shop/t/3/assets/product-card.js.map?v=174129639976667491711754935550
