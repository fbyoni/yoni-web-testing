export class PaginatedListAspectRatioHelper {
  ...
  constructor({ templateCard }) {
    Shopify.designMode && this.#storeImageRatioSettings(templateCard);
  }
  processNewElements() {
    Shopify.designMode &&
  ...
  #fixAdaptiveAspectRatios() {
    ...
    newCardGalleries.forEach((gallery) => {
      ...
      const loadAndSetRatio = () => {
        ...
        (productId && productRatioCache.set(productId, imgRatio),
          this.#applyAspectRatioToGallery(gallery, imgRatio));
      };
      img.complete
        ? loadAndSetRatio()
        : img.addEventListener("load", loadAndSetRatio, { once: !0 });
    });
  }
  #applyFixedAspectRatio() {
    ...
    const newCardGalleries = this.#getUnprocessedGalleries();
    newCardGalleries.length &&
      requestAnimationFrame(() => {
        newCardGalleries.forEach((gallery) => {
          gallery instanceof HTMLElement && this.#applyAspectRatioToGallery(gallery, aspectRatio);
        });
      });
  }
  ...
  #applyAspectRatioToGallery(gallery, aspectRatio) {
    ...
    (gallery.style.setProperty("--gallery-aspect-ratio", aspectRatio),
      gallery.querySelectorAll(".product-media-container").forEach((container) => {
        container instanceof HTMLElement && (container.style.aspectRatio = aspectRatio);
      }),
  ...
  #markAsProcessed(gallery) {
    gallery instanceof HTMLElement && gallery.setAttribute("data-aspect-ratio-applied", "true");
  }
}
//# sourceMappingURL=/cdn/shop/t/3/assets/paginated-list-aspect-ratio.js.map?v=25254270272143792451754421400
