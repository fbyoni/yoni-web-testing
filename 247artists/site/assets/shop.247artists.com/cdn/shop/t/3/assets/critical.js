...
export class OverflowMinimumEvent extends Event {
  constructor(minimumReached) {
    (super("overflowMinimum", { bubbles: !0 }), (this.detail = { minimumReached }));
  }
}
export class ReflowEvent extends Event {
  constructor(lastVisibleElement) {
    (super("reflow", { bubbles: !0 }), (this.detail = { lastVisibleElement }));
  }
}
export class OverflowList extends DeclarativeShadowElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    name === "disabled" && (newValue === "true" ? this.#reset() : this.#reflowItems());
  }
  connectedCallback() {
    if ((super.connectedCallback(), this.hasAttribute("defer"))) {
      const deferredReflow = () => {
          (this.removeAttribute("defer"), this.#initialize());
        },
        { schedule } = this;
      (typeof window.requestIdleCallback == "function" ? window.requestIdleCallback : setTimeout)(
        () => schedule(deferredReflow),
      );
    } else
      this.shadowRoot
        ? this.#initialize()
  ...
  #initialize() {
    ...
    const defaultSlot = shadowRoot.querySelector("slot:not([name])"),
      overflowSlot = shadowRoot.querySelector('slot[name="overflow"]'),
      moreSlot = shadowRoot.querySelector('slot[name="more"]'),
      overflow = shadowRoot.querySelector('[part="overflow"]'),
      list = shadowRoot.querySelector('[part="list"]'),
      placeholder = shadowRoot.querySelector('[part="placeholder"]');
    if (
    ...
    ((this.#refs = { defaultSlot, overflowSlot, moreSlot, overflow, list, placeholder }),
  ...
  get schedule() {
    return typeof Theme?.utilities?.scheduler?.schedule == "function"
      ...
      : (callback) => requestAnimationFrame(() => setTimeout(callback, 0));
  }
  #scheduled = !1;
  get minimumItems() {
  ...
  #lastDimensions = { width: null, height: null };
  #handleChange = (event) => {
    ...
    ((this.#scheduled = !0),
      this.schedule(() => {
        (this.#reflowItems(), (this.#scheduled = !1));
      }));
  };
  #moveItemsToDefaultSlot() {
    const { defaultSlot, overflowSlot } = this.#refs;
    for (const element of overflowSlot.assignedElements()) element.slot = defaultSlot.name;
  }
  #reset() {
  ...
  #reflowItems = (lastVisibleElement) => {
    ...
    if (visibleElements.length !== elements.length || lastVisibleElement) {
      if (
        ...
        lastVisibleElement && lastVisibleElement.style.setProperty("order", "-1"),
        (visibleElements = getVisibleElements()),
        moreSlot.style.removeProperty("order"),
        lastVisibleElement && lastVisibleElement.style.removeProperty("order"),
        lastVisibleElement)
      ) {
        const visibleIndex = visibleElements.indexOf(lastVisibleElement);
        visibleIndex !== -1 &&
          (visibleElements.splice(visibleIndex, 1), visibleElements.push(lastVisibleElement));
      }
      moreSlot.style.setProperty("height", "auto");
    }
    const overflowingElements = elements.filter((element) => !visibleElements.includes(element)),
      [firstOverflowingElement] = overflowingElements,
      hasOverflow = overflowingElements.length > 0,
      placeholderWidth = firstOverflowingElement ? firstOverflowingElement.clientWidth : 0;
    for (const element of elements)
      element.slot = overflowingElements.includes(element) ? overflowSlot.name : defaultSlot.name;
    ((moreSlot.hidden = !hasOverflow),
      hasOverflow &&
        ((placeholder.style.width = `${placeholderWidth}px`), (placeholder.hidden = !1)),
...
customElements.get("overflow-list") || customElements.define("overflow-list", OverflowList);
...
((() => {
  ...
  const headerHeight = header.offsetHeight,
    headerGroupHeight = calculateHeaderGroupHeight(header);
  (document.body.style.setProperty("--header-height", `${headerHeight}px`),
    document.body.style.setProperty("--header-group-height", `${headerGroupHeight}px`));
})(),
  (() => {
    const header = document.querySelector("#header-component"),
      hasHeaderSection = document.querySelector("#header-group")?.querySelector(".header-section");
    if (!hasHeaderSection || !header?.hasAttribute("transparent")) {
      document.body.style.setProperty("--transparent-header-offset-boolean", "0");
      return;
    }
    const shouldApplyOffset = hasHeaderSection.nextElementSibling?.classList.contains(
      "shopify-section",
    )
      ? "0"
      : "1";
...
//# sourceMappingURL=/cdn/shop/t/3/assets/critical.js.map?v=34395725382898153171754421401
