(function () {
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    ...
    viewTransitionRenderBlocker && viewTransitionRenderBlocker.remove();
  }
  ...
  (window.addEventListener("pageswap", async (event) => {
    ...
    const transitionType = document
      .querySelector("[data-view-transition-triggered]")
      ?.getAttribute("data-view-transition-type");
    transitionType
    ...
    window.addEventListener("pagereveal", async (event) => {
      ...
      customTransitionType
        ...
        : (viewTransition.types.clear(), viewTransition.types.add("page-navigation"));
    }));
  function hasViewTransition(event) {
    return "viewTransition" in event && event.viewTransition instanceof ViewTransition;
  }
})();
//# sourceMappingURL=/cdn/shop/t/3/assets/view-transitions.js.map?v=84877080446900032951754421401
