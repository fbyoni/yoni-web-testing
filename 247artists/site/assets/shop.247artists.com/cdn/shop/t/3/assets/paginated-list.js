...
export default class PaginatedList extends Component {
  ...
  connectedCallback() {
    ...
    (templateCard &&
      (this.#aspectRatioHelper = new PaginatedListAspectRatioHelper({ templateCard })),
      this.#fetchPage("next"),
  ...
  disconnectedCallback() {
    (super.disconnectedCallback(),
      this.infinityScrollObserver && this.infinityScrollObserver.disconnect(),
          ...
          async (entries) => {
            ...
            for (const entry of entries)
              if (entry.isIntersecting) {
                const { viewMorePrevious: viewMorePrevious2, viewMoreNext: viewMoreNext2 } =
                  this.refs;
                entry.target === viewMorePrevious2
                  ? this.#renderPreviousPage()
                  : entry.target === viewMoreNext2 && this.#renderNextPage();
              }
          },
          { rootMargin: "100px" },
        )),
      viewMorePrevious && this.infinityScrollObserver.observe(viewMorePrevious),
      viewMoreNext && this.infinityScrollObserver.observe(viewMoreNext));
  }
  #shouldUsePage(pageInfo) {
    if (!pageInfo) return !1;
    const { grid } = this.refs,
      lastPage = grid?.dataset.lastPage;
    return !(!lastPage || pageInfo.page < 1 || pageInfo.page > Number(lastPage));
  }
  async #fetchPage(type) {
    const page = this.#getPage(type),
      resolvePromise = () => {
        type === "next"
          ? (this.#resolveNextPagePromise?.(), (this.#resolveNextPagePromise = null))
          : (this.#resolvePreviousPagePromise?.(), (this.#resolvePreviousPagePromise = null));
      };
    if (!page || !this.#shouldUsePage(page)) {
      resolvePromise();
      return;
    }
    (await this.#fetchSpecificPage(page.page, page.url), resolvePromise());
  }
  async #fetchSpecificPage(pageNumber, url = void 0) {
    ...
    const pageContent = await sectionRenderer.getSectionHTML(this.sectionId, !0, pageInfo.url);
  ...
  async #renderPreviousPage() {
    ...
    if (!previousPageItemElements) {
      ...
      if (
        ...
        !previousPageItemElements)
      )
        return;
    }
    const scrollTop = window.scrollY,
      firstElement = grid.firstElementChild,
      oldHeight = firstElement ? firstElement.getBoundingClientRect().top + window.scrollY : 0;
    if (
      ...
      const heightDiff = firstElement.getBoundingClientRect().top + window.scrollY - oldHeight;
  ...
  #getPage(type) {
    ...
    const currentCardPage = Number(targetCard.dataset.page),
      page = isPrevious ? currentCardPage - 1 : currentCardPage + 1,
      url = new URL(window.location.href);
    return (url.searchParams.set("page", page.toString()), (url.hash = ""), { page, url });
  }
  #getGridForPage(page) {
    ...
    if (gridElement) return gridElement.querySelectorAll(':scope > [ref="cards[]"]');
  }
  get sectionId() {
    ...
    const currentLastPage = this.refs.grid?.dataset.lastPage,
      observer = new MutationObserver(() => {
        if (this.refs.grid?.dataset.lastPage !== currentLastPage) {
          if ((observer.disconnect(), !this.isConnected)) return;
          (this.#observeViewMore(), this.#fetchPage("next"));
        }
      }),
      { grid } = this.refs;
    grid &&
...
//# sourceMappingURL=/cdn/shop/t/3/assets/paginated-list.js.map?v=95298315036233333871754421400
