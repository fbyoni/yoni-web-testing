...
class HeaderDrawer extends Component {
  ...
  disconnectedCallback() {
    (super.disconnectedCallback(), this.removeEventListener("keyup", this.#onKeyUp));
  }
  #onKeyUp = (event) => {
    event.key === "Escape" && this.#close(this.#getDetailsElement(event));
  };
  get isOpen() {
  ...
  #getDetailsElement(event) {
    return event?.target instanceof Element
      ? (event.target.closest("details") ?? this.refs.details)
      : this.refs.details;
  }
  toggle() {
      ...
      onAnimationEnd(details, () => {
        (reset(details),
          details === this.refs.details
            ? (removeTrapFocus(), this.querySelectorAll("details[open]").forEach(reset))
            : setTimeout(() => {
  ...
  #setupAnimatedElementListeners() {
    ...
    this.querySelectorAll(".menu-drawer__animated-element").forEach((element) => {
      element.addEventListener("animationend", removeWillChangeOnAnimationEnd);
    });
  }
}
customElements.get("header-drawer") || customElements.define("header-drawer", HeaderDrawer);
function reset(element) {
  (element.classList.remove("menu-open"),
    element.removeAttribute("open"),
    element.querySelector("summary")?.setAttribute("aria-expanded", "false"));
}
//# sourceMappingURL=/cdn/shop/t/3/assets/header-drawer.js.map?v=38290796699993510201754935553
