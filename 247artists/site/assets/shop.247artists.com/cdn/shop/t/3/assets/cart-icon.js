...
class CartIcon extends Component {
  ...
  onCartUpdate = async (event) => {
    const itemCount = event.detail.data?.itemCount ?? 0,
      comingFromProductForm = event.detail.data?.source === "product-form-component";
    this.renderCartBubble(itemCount, comingFromProductForm);
  };
  renderCartBubble = async (itemCount, comingFromProductForm, animate = !0) => {
    (this.refs.cartBubbleCount.classList.toggle("hidden", itemCount === 0),
      ...
      sessionStorage.setItem(
        "cart-count",
        JSON.stringify({ value: String(this.currentCartCount), timestamp: Date.now() }),
      ),
      animate &&
  ...
  ensureCartBubbleIsCorrect = () => {
    const sessionStorageCount = sessionStorage.getItem("cart-count"),
      visibleCount = this.refs.cartBubbleCount.textContent;
    if (!(sessionStorageCount === visibleCount || sessionStorageCount === null))
      try {
        const { value, timestamp } = JSON.parse(sessionStorageCount);
        if (Date.now() - timestamp < 1e4) {
          const count = parseInt(value, 10);
          count >= 0 && this.renderCartBubble(count, !1, !1);
        }
      } catch {}
  };
}
customElements.get("cart-icon") || customElements.define("cart-icon", CartIcon);
//# sourceMappingURL=/cdn/shop/t/3/assets/cart-icon.js.map?v=14733679814066288171754421400
