...
class PredictiveSearchComponent extends Component {
  ...
  connectedCallback() {
    ...
    (this.refs.searchInput.value.length > 0 && this.#showResetButton(),
      dialog
  ...
  #handleKeyboardShortcut = (event) => {
    event.metaKey && event.key === "k" && this.dialog?.toggleDialog();
  };
  #handleDialogClose = () => {
    this.#resetSearch();
  };
  expandSearch = () => {
    (this.classList.add("predictive-search--expanded"),
      this.dataset.activeColorScheme &&
        (this.dialog ?? this).classList.add(`color-${this.dataset.activeColorScheme}`));
  };
  get #allResultsItems() {
    return Array.from(
      this.querySelectorAll(
        ".predictive-search-results__wrapper-queries, .predictive-search-results__wrapper-products, .predictive-search-results__list",
      ),
    )
      .flatMap((container) =>
        container.classList.contains("predictive-search-results__wrapper-products")
          ? Array.from(container.querySelectorAll(".predictive-search-results__card"))
          : Array.from(
              container.querySelectorAll(
                '[ref="resultsItems[]"], .predictive-search-results__card',
              ),
            ),
      )
  ...
  get #currentIndex() {
    return (
      this.#allResultsItems?.findIndex((item) => item.getAttribute("aria-selected") === "true") ??
      -1
    );
  }
  set #currentIndex(index) {
    if (this.#allResultsItems?.length) {
      ...
      for (const [itemIndex, item] of this.#allResultsItems.entries())
        itemIndex === index
          ? (item.setAttribute("aria-selected", "true"),
            this.#isKeyboardNavigation && item.classList.add("keyboard-focus"),
            item.scrollIntoView({
  ...
  onSearchKeyDown = (event) => {
    ...
    const currentIndex = this.#currentIndex,
      totalItems = this.#allResultsItems.length;
    switch (event.key) {
      case "ArrowDown":
      ...
      case "Enter": {
        ...
        if (this.#currentIndex >= 0)
          (event.preventDefault(), this.#currentItem?.querySelector("a")?.click());
        else {
  ...
  clearRecentlyViewedProducts(event) {
    (event.stopPropagation(), RecentlyViewed.clearProducts());
    const { recentlyViewedItems, recentlyViewedTitle, recentlyViewedWrapper } = this.refs;
    [...(recentlyViewedItems || []), ...(recentlyViewedTitle || [])].length !== 0 &&
  ...
  resetSearch = debounce((keepFocus = !0) => {
    (keepFocus && this.refs.searchInput.focus(), this.#resetSearch());
  }, 100);
  search = debounce((event) => {
    if (!event.inputType) return;
    const searchTerm = this.refs.searchInput.value.trim();
    if (((this.#currentIndex = -1), !searchTerm.length)) {
      this.#resetSearch();
      return;
    }
    (this.#showResetButton(), this.#getSearchResults(searchTerm));
  }, 200);
  #resetScrollPositions() {
    requestAnimationFrame(() => {
      const resultsInner = this.refs.predictiveSearchResults.querySelector(
        ".predictive-search-results__inner",
      );
      resultsInner instanceof HTMLElement && (resultsInner.scrollTop = 0);
      const formContent = this.querySelector(".predictive-search-form__content");
      formContent instanceof HTMLElement && (formContent.scrollTop = 0);
    });
  }
  async #getSearchResults(searchTerm) {
    if (!this.dataset.sectionId) return;
    const url = new URL(Theme.routes.predictive_search_url, location.origin);
    (url.searchParams.set("q", searchTerm), url.searchParams.set("resources[limit_scope]", "each"));
    const { predictiveSearchResults } = this.refs,
      abortController = this.#createAbortController();
    sectionRenderer
      .getSectionHTML(this.dataset.sectionId, !1, url)
      .then((resultsMarkup) => {
        resultsMarkup &&
          (abortController.signal.aborted ||
            (morph(predictiveSearchResults, resultsMarkup), this.#resetScrollPositions()));
      })
      .catch((error) => {
  ...
  async #getRecentlyViewedProductsMarkup() {
    ...
    return (
      ...
      sectionRenderer.getSectionHTML(this.dataset.sectionId, !1, url)
    );
  }
  #getRecentlyViewed = async () => {
    const { predictiveSearchResults } = this.refs,
      abortController = this.#createAbortController(),
      resultsMarkup = await this.#getRecentlyViewedProductsMarkup();
    if (!resultsMarkup) return;
    const recentlyViewedProductsHtml = new DOMParser()
      .parseFromString(resultsMarkup, "text/html")
  ...
  #resetSearch = async () => {
    const { predictiveSearchResults, searchInput } = this.refs,
      emptySectionId = "predictive-search-empty";
    ((this.#currentIndex = -1), (searchInput.value = ""), this.#hideResetButton());
    const abortController = this.#createAbortController(),
      emptySectionMarkup = await sectionRenderer.getSectionHTML(emptySectionId, !1),
      parsedEmptySectionMarkup = new DOMParser()
    ...
    abortController.signal.aborted ||
      (morph(predictiveSearchResults, parsedEmptySectionMarkup), this.#resetScrollPositions());
  };
  #blurSearch = () => {
    this.refs.searchInput.blur();
  };
}
customElements.get("predictive-search-component") ||
  customElements.define("predictive-search-component", PredictiveSearchComponent);
//# sourceMappingURL=/cdn/shop/t/3/assets/predictive-search.js.map?v=137734153844302985541754421401
