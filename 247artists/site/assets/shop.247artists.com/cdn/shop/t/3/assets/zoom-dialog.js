...
export class ZoomDialog extends Component {
  ...
  connectedCallback() {
    (super.connectedCallback(), this.refs.dialog.addEventListener("scroll", this.handleScroll));
  }
  ...
  async open(index, event) {
    ...
    const { dialog, media, thumbnails } = this.refs,
      targetImage = media[index],
      targetThumbnail = thumbnails.children[index],
      open = () => {
        dialog.showModal();
  ...
  #loadHighResolutionImage(mediaContainer) {
    if (!mediaContainer.classList.contains("product-media-container--image")) return !1;
    const image = mediaContainer.querySelector("img.product-media__image");
    if (!image || !(image instanceof HTMLImageElement)) return !1;
    const highResolutionUrl = image.getAttribute("data_max_resolution");
    if (!highResolutionUrl || this.#highResImagesLoaded.has(highResolutionUrl)) return !1;
    ...
    ((newImage.className = image.className),
      ...
      (newImage.onload = () => {
        (image.replaceWith(newImage), this.#highResImagesLoaded.add(highResolutionUrl));
      }),
      (newImage.src = highResolutionUrl));
  }
  handleScroll = debounce(async () => {
    const { media, thumbnails } = this.refs,
      mostVisibleElement = await getMostVisibleElement(media),
      activeIndex = media.indexOf(mostVisibleElement),
      targetThumbnail = thumbnails.children[activeIndex];
    !targetThumbnail ||
      !(targetThumbnail instanceof HTMLElement) ||
  ...
  async close() {
    ...
    const mostVisibleElement = await getMostVisibleElement(media),
      activeIndex = media.indexOf(mostVisibleElement),
      transitionName = "gallery-item",
      mediaGallery = this.closest("media-gallery"),
      slide =
        mediaGallery?.presentation === "carousel"
  ...
  closeDialog() {
    ...
    (dialog.close(), window.dispatchEvent(new DialogCloseEvent()));
  }
  handleKeyDown(event) {
    event.key === "Escape" && (event.preventDefault(), this.close());
  }
  ...
  async handleThumbnailPointerEnter(index) {
    ...
    media[index] && this.#loadHighResolutionImage(media[index]);
  }
  async selectThumbnail(index, options = { behavior: "smooth" }) {
    if (
      !this.refs.thumbnails ||
      !this.refs.thumbnails.children.length ||
      isNaN(index) ||
      index < 0 ||
...
customElements.get("zoom-dialog") || customElements.define("zoom-dialog", ZoomDialog);
  ...
  return new Promise((resolve) => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        (observer.disconnect(), resolve(mostVisible.target));
      },
...
//# sourceMappingURL=/cdn/shop/t/3/assets/zoom-dialog.js.map?v=67809429400448230651754421401
