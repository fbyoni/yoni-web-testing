  ...
  [1776],
  {
    1776: (t, i, e) => {
      ...
      var s = e(8760),
        ...
        r = e(314),
        n = e(8188);
      (h.w$.registerPlugin(r.c, n.c),
        (window.plr.controllers.SCtaHome = class extends s.c {
          init() {
            super.init();
          ...
          setSizes() {
            (this.setCanvasSize(), this.setGraphics(), this.setClipPaths());
          }
          setClipPaths() {
            const { intro: t } = this.refs;
            ((this.clipPaths = []), (this.clipPathWidth = t.offsetWidth / 6));
            const i = t.offsetHeight;
            t.style.setProperty("--strip-width", `${this.clipPathWidth}px`);
            for (let t = 0; t < 6; t++)
              this.clipPaths.push({ x: t * this.clipPathWidth, height: i });
          }
          setCanvasSize() {
            const { canvas: t, scene: i } = this.refs,
          ...
          setLines() {
            ...
            for (let n = 0; n <= 10; n++)
              for (let o = 0; o < 4; o++) {
                let a = "p1";
                2 === o ? (a = "p4") : 3 === o && (a = "p2");
                const c = { x: t[a].x + e * n * ((o + 1) % 2), y: t[a].y + s * n * (o % 2) },
                  d = { x: i[a].x + h * n * ((o + 1) % 2), y: i[a].y + r * n * (o % 2) };
          ...
          initScroll() {
            const { introTitle: t, title: i } = this.refs,
              { innerRect: e } = this;
            (this.tl && this.tl.revert(),
              this.titleSt && this.titleSt.revert(),
              this.introTitleSt && this.introTitleSt.revert(),
              ...
              this.tl.fromTo(t, { y: "300%" }, { y: "-300%", ease: "linear", duration: 3.5 }, 0),
              this.tl.from(
                this.titleSt.lines,
                {
                  y: -0.5 * this.render.height,
                  ease: "power2.out",
                  duration: 2.75,
              ...
              this.tl.fromTo(
                e,
                { offset: -0.5 * this.render.height },
                { offset: 0, ease: "power2.out", duration: 2.5 },
                1.75,
              ),
              this.tl.to(
                this.clipPaths,
          ...
          drawClipPaths() {
            const { intro: t } = this.refs,
              { clipPathWidth: i } = this;
            let e = "";
            (this.clipPaths.forEach((t) => {
              e += `M ${t.x} 0 h ${i} v ${t.height} h -${i}  Z `;
            }),
              t.style.setProperty("--clip-path", `path('${e}')`));
          }
          tick(t) {
            if (this.isPaused) return;
            const { canvas: i } = this.refs,
              { ctx: e, innerRect: s, rects: h, render: r } = this;
            (e.clearRect(0, 0, i.width, i.height),
              ...
              this.drawLines());
            const n = [...this.images].reverse();
            (n.forEach((t) => {
              (t.move(), t.draw());
            }),
              Math.random() < 0.1 && t - this.lastEmit > 1e3 && n.length < 5 && this.emitImage(),
              this.drawClipPaths(),
      ...
      class o {
        ...
        draw() {
          const { isLoaded: t, image: i, scale: e, visibility: s } = this,
            h = this.parent.ctx,
            r = this.destinationSize;
          if (!r) return;
          const n = this.sourceSize,
            o = r.width * e,
            a = r.height * e,
            c = r.x - (o - r.width) / 2,
            d = r.y + this.parent.innerRect.offset * (1 - s);
          t &&
            (h.save(),
            h.beginPath(),
            this.clipPaths.forEach((t) => {
              const i = c + ((r.width - t.width) / 2) * e,
                s = d + t.y * e,
                n = t.width * e,
            ...
            h.drawImage(i, n.x, n.y, n.width, n.height, c, d, o, a),
            h.restore());
        }
        destroy() {
      ...
      class a {
        ...
        draw() {
          const { ctx: t, innerRect: i, outerRect: e } = this,
            ...
            r = { x: i.p2.x + (e.p2.x - i.p2.x) * s, y: i.p2.y + (e.p2.y - i.p2.y) * s + i.offset },
            n = { x: i.p3.x + (e.p3.x - i.p3.x) * s, y: i.p3.y + (e.p3.y - i.p3.y) * s + i.offset },
