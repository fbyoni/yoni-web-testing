...
(function () {
  ...
  function parsePriceText(txt) {
    if (!txt) return null;
    var m = String(txt).replace(/,/g, '').match(/(\d+(?:\.\d{1,2})?)/);
    if (!m) return null;
  ...
  function extractItem(form) {
    ...
    var checked = scope.querySelector('variant-picker input[type="radio"]:checked, fieldset input[type="radio"]:checked');
    ...
    var priceScope =
      ...
      scope;
    var priceEl = priceScope && priceScope.querySelector ? priceScope.querySelector('.price, .price-item, [class*="price"]') : null;
    if (!priceEl) {
    ...
    var image = '';
    var addComp = form.querySelector('add-to-cart-component[data-product-variant-media]') ||
    ...
    var url = '';
    var link = (closest(form, '.product-card') || document).querySelector('a.product-card__link, a[href*="/shop/products/"]');
      ...
      body.innerHTML = cart
        .map(function (it, idx) {
          ...
          return (
            ...
            '</div>' +
            '</div>' +
            '<div class="mock-line-total">' + MONEY(it.price * it.qty) + '</div>' +
            '</div>'
          );
        })
        .join('');
  ...
  function doCheckout() {
    ...
    modal.innerHTML =
      ...
      '<p class="mock-checkout-total">Total paid: <strong>' + MONEY(subtotal(cart)) + '</strong></p>' +
